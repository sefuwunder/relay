...
- Unanswered invitation cards (yours or inbound) carry a small **✕ remove** button, and invitations nobody answered **drop silently after 24 hours** — no notification, no chime, no log. Accepted, declined, cancelled, and diary entries are never swept. New API: `DELETE /api/conversations/:id/appointments/:appointmentId` (pending invitations only).
