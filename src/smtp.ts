...
export interface SendMailOpts {
  ...
  inReplyTo?: string;
  /** Outbound Message-ID for this send. Generated by the caller (see
      newMessageId) so it can be stored and later matched against the copy
      that lands in the Sent folder — that is how sent-mail import dedupes
      against mail composed inside Relay. */
  messageId?: string;
...
/** A fresh, unique Message-ID for an outbound send. */
...
export function buildMessage(cfg: SmtpConfig, opts: SendMailOpts): string {
  ...
  if (opts.messageId) {
    const clean = opts.messageId.replace(/[<>\r\n\s]/g, "").slice(0, 120);
    if (clean) headers.push(`Message-ID: <${clean}>`);
  }
  if (opts.inReplyTo) {
